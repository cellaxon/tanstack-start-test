...
function TracesPage() {
	...
	const filteredTraces = useMemo(() => {
		...
		return tracesQuery.data.traces.filter(
			(trace) =>
						...
						<CardDescription>
							...
							{tracesQuery.data && (
								<span className="ml-2 text-xs">
									({filteredTraces.length} of {tracesQuery.data.traces.length}{" "}
									traces)
								</span>
							)}
						</CardDescription>
					...
					<Card>
						...
						<CardContent>
							<p className="text-2xl font-bold">
								{traceDetailsQuery.data.spanCount}
							</p>
						</CardContent>
					</Card>
					<Card>
						...
						<CardContent>
							<p className="text-2xl font-bold">
								{traceDetailsQuery.data.serviceCount}
							</p>
						</CardContent>
					</Card>
					<Card>
						...
						<CardContent>
							<p className="text-2xl font-bold">
								{traceDetailsQuery.data.duration}ms
							</p>
						</CardContent>
					</Card>
					<Card>
						...
						<CardContent>
							<p
								className={`text-2xl font-bold ${
									traceDetailsQuery.data.errorCount > 0
										? "text-red-500"
										: "text-green-500"
								}`}
							>
			...
			{import.meta.env.DEV && (
				<div className="fixed bottom-4 right-4 bg-background border rounded-lg p-2 text-xs space-y-1 opacity-50 hover:opacity-100 transition-opacity">
					...
					{tracesQuery.dataUpdatedAt && (
						<div>
							Updated:{" "}
							{new Date(tracesQuery.dataUpdatedAt).toLocaleTimeString()}
						</div>
					)}
				</div>
			)}
