...
function ApiDemoPage() {
	...
	const weatherQuery: MockQuery = {
		data: null,
		isLoading: false,
		error: null,
		refetch: () => {},
	};
	const githubQuery: MockQuery = {
		data: null,
		isLoading: false,
		error: null,
		refetch: () => {},
	};
	const newsQuery: MockQuery = {
		data: null,
		isLoading: false,
		error: null,
		refetch: () => {},
	};
	const cryptoQuery: MockQuery = { data: null, isLoading: false, error: null };
	const ipQuery: MockQuery = { data: null, isLoading: false, error: null };
	const webhookMutation: MockMutation = { mutate: () => {}, isPending: false };
		...
		<div className="container mx-auto p-6 space-y-6">
			<div className="mb-6">
				...
				<p className="text-gray-600 mt-2">
					Examples of integrating with various external APIs using TanStack
					Query
				</p>
			</div>
			...
			<div className="grid gap-6 md:grid-cols-2">
					...
					<CardContent className="space-y-4">
						...
						{newsQuery.data && (
							<div className="space-y-2 max-h-64 overflow-y-auto">
								{newsQuery.data
									.slice(0, 3)
									.map((article: { url: string; title: string; description: string }, index: number) => (
										<div key={`article-${index}`} className="p-3 bg-gray-50 rounded-lg">
											<a
												href={article.url}
												target="_blank"
												rel="noopener noreferrer"
				...
				<Card>
					...
					<CardContent className="space-y-4">
						...
						{cryptoQuery.data && (
							<div className="space-y-2 max-h-64 overflow-y-auto">
								{cryptoQuery.data.slice(0, 5).map((crypto: { id: string; image: string; name: string; symbol: string; current_price: number; price_change_percentage_24h: number }) => (
				...
				<Card className="md:col-span-2">
					...
					<CardContent className="space-y-4">
						...
						<p className="text-sm text-gray-500">
							Note: This will attempt to send a test payload to a webhook
							endpoint. Replace the URL in the code with your actual webhook
							endpoint.
						</p>
					</CardContent>
				</Card>
