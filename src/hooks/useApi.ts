import { useQuery, useMutation, useInfiniteQuery, type UseQueryOptions, type UseMutationOptions } from '@tanstack/react-query';
...
export function useResourceCRUD<T>(resourceName: string) {
  ...
  return {
    ...
    useUpdate: (id: string | number, options?: UseMutationOptions<T, Error, Partial<T>>) =>
      useApiMutation<T, Partial<T>>(`${baseEndpoint}/${id}`, 'PUT', options),

    usePatch: (id: string | number, options?: UseMutationOptions<T, Error, Partial<T>>) =>
      useApiMutation<T, Partial<T>>(`${baseEndpoint}/${id}`, 'PATCH', options),

    useDelete: (id: string | number, options?: UseMutationOptions<void, Error, void>) =>
