...
class AuthService {
  ...
  static async login(provider: 'google' | 'github' | 'microsoft', code: string): Promise<TokenResponse> {
  ...
  static getOAuthUrl(provider: 'google' | 'github' | 'microsoft'): string {
    ...
    const clientIds = {
      ...
      microsoft: import.meta.env.VITE_MICROSOFT_CLIENT_ID || 'demo-microsoft-client-id',
    };
