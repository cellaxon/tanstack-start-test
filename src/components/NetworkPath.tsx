						...
						<div className="flex justify-between">
							...
							<span
								className={`font-semibold ${
									parseFloat(nodeData.status.load) > 80
										? "text-red-500"
										: parseFloat(nodeData.status.load) > 50
											? "text-yellow-500"
											: "text-green-500"
								}`}
							>
		...
		async (preservePositions = false) => {
			try {
				...
				setNodes((currentNodes) => {
					...
					// Transform nodes
					const transformedNodes: NetworkNode[] = data.nodes.map(
						(node: { id: string; x: number; y: number; label: string; type: string }) => {
							// Use existing position if available and preservePositions is true
							const savedPosition = currentPositions.get(node.id);
							const position =
								preservePositions && savedPosition
									? savedPosition
									: { x: node.x, y: node.y };
				...
				// Transform edges
				const transformedEdges: NetworkEdge[] = data.edges.map((edge: { id: string; source: string; target: string; label?: string; animated?: boolean }) => ({
					...edge,
					type: "custom",
					markerEnd: {
				...
				<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
					...
					<Card>
						<CardContent className="p-4">
							...
							<div className="text-xl font-bold">
								{metrics.bandwidth.inbound} Mbps
							</div>
						</CardContent>
					</Card>
					<Card>
						<CardContent className="p-4">
							...
							<div className="text-xl font-bold">
								{metrics.bandwidth.outbound} Mbps
							</div>
						</CardContent>
					</Card>
				</div>
